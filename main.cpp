...
args_t parse_args(int argc, char *argv[]) {
    ...
    args_t args = {0};
    optind = 0;
    while ((opt = getopt_long_only(argc, argv, "o:s:a::h", long_options,
                                   NULL)) != -1) {
        switch (opt) {
        case 0:
            break;
        case 'o':
            ...
            break;
        case 's':
            args.scheme = strdup(optarg);
            break;
        case 'a':
            args.inst_args = strdup(optarg);
            break;
        case 'h':
        default:
            ERR("Usage: %s [--trace (opt)] [--multithread (opt)] [--time "
                "(opt)] "
                "[--scheme SCHEME] [--args SCHEME_ARGS (opt)] [--out OUTFILE] "
                "input-file\n",
                argv[0]);
            ERR("Supported schemes: \'empty\', \'sample\', \'loop-count\', "
                "\'memaccess-stochastic\', \'r3-record\', "
                "\'allspark-trace\'\n");
            exit(opt != 'h');
        }
    }
...
static void encode_and_write_modules(char *outfile,
...
std::vector<WasmModule> instrument_call(WasmModule &module, std::string routine,
                                        ...
                                        bool &is_batch, bool &uses_encode) {
    ...
    if (routine == "empty") {
        ...
        out_modules = memaccess_stochastic_instrument(
            module, percent, cluster_size, path, encode_and_write_single);
        is_batch = true;
        uses_encode = true;
    }
    ...
    else if (routine == "sample") {
        sample_instrument(module);
    } else if (routine == "func-weight") {
        all_funcs_weight_instrument(module);
    } else if (routine == "func-entry") {
        func_entry_instrument(module);
    } else if (routine == "loop-count") {
        loop_instrument(module);
    } else if (routine == "opcode-count") {
        opcode_count_instrument(module);
    } else if (routine == "r3-record") {
        ...
        std::string entry_export =
            ((args.size() == 1) ? args[0] : start_export_str);
        allspark_trace_instrument(module, entry_export);
    } else if (routine == "snapshot") {
        snapshot_instrument(module);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    std::string inst_args =
        (args.inst_args ? std::string(args.inst_args) : std::string());
    ...
    TIME_SECTION(0, "Time to instrument",
                 std::vector<WasmModule> out_modules = instrument_call(
                     module, args.scheme, arg_vec, is_batch, uses_encode);)

    /* Encode instrumented module */
    TIME_SECTION(
        0, "Time to encode modules",
        if (!is_batch) {
