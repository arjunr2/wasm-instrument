...
wasm_instrument_mod_t decode_instrument_module(byte *file_buf,
                                               uint32_t file_size);

byte *encode_file_buf_from_module(wasm_instrument_mod_t mod,
                                  uint32_t *file_size);

/** Instrumentation (in-place) **/
void instrument_module(wasm_instrument_mod_t mod, const char *routine,
...
byte *instrument_module_buffer(byte *inbuf, uint32_t insize, uint32_t *outsize,
                               const char *routine, void *args,
                               uint32_t num_args, int64_t flags);

/** Deep-Copy modules **/
wasm_instrument_mod_t copy_instrument_module(wasm_instrument_mod_t mod);

/** Free methods **/
void destroy_instrument_module(wasm_instrument_mod_t mod);

void destroy_file_buf(byte *file_buf);
