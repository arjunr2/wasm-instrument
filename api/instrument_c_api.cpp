...
typedef enum { StringArgs, AnonArrArgs } InstrumentArgType;
...
void allspark_trace_insthandle(WasmModule &module, ArgVec args) {
    std::string entry_export =
        (args.empty() ? start_export_str : POP_STR(args));
...
static std::vector<routines_t> inst_routines = {
    {"memaccess-stochastic", 2, 3, StringArgs,
     InstrumentFn{.string_arg = memaccess_stochastic_insthandle}},
    {"memaccess-balanced", 2, 3, StringArgs,
     InstrumentFn{.string_arg = memaccess_balanced_insthandle}},
    {"sample", 0, 0, StringArgs, InstrumentFn{.string_arg = sample_insthandle}},
    {"func-weight", 0, 0, StringArgs,
     InstrumentFn{.string_arg = func_weight_insthandle}},
    {"loop-count", 0, 0, StringArgs,
     InstrumentFn{.string_arg = loop_count_insthandle}},
    {"opcode-count", 0, 0, StringArgs,
     InstrumentFn{.string_arg = opcode_count_insthandle}},
    {"r3-record", 0, 0, StringArgs,
     InstrumentFn{.string_arg = r3_record_insthandle}},
    {"r3-replay-generator", 0, UINT32_MAX, AnonArrArgs,
     InstrumentFn{.anonarr_arg = r3_replay_instrument}},
    {"allspark-trace", 0, 1, StringArgs,
     InstrumentFn{.string_arg = allspark_trace_insthandle}},
    {"empty", 0, 0, StringArgs, InstrumentFn{.string_arg = empty_insthandle}}};

WasmModule *decode_instrument_module(byte *file_buf, uint32_t file_size) {
    WasmModule *module = new WasmModule();
    *module = parse_bytecode(file_buf, file_buf + file_size);
    return module;
}

byte *encode_file_buf_from_module(WasmModule *module, uint32_t *file_size) {
...
void instrument_module(WasmModule *mod, const char *scheme, void *args,
                       uint32_t num_args, int64_t flags) {
    bool match = false;
    for (auto &routine : inst_routines) {
        if (routine.name == std::string(scheme)) {
            match = true;
            if ((num_args < routine.min_args) ||
                (num_args > routine.max_args)) {
                printf("%s scheme takes %d <= n <= %d args (provided %d)\n",
                       routine.name, routine.min_args, routine.max_args,
                       num_args);
                ...
                if (routine.args_type == StringArgs) {
                    ...
                    throw std::runtime_error(
                        "Invalid instrumentation args type");
                }
            }
        }
    }
...
byte *instrument_module_buffer(byte *inbuf, uint32_t insize, uint32_t *outsize,
                               const char *scheme, void *args,
                               uint32_t num_args, int64_t flags) {
...
/** Deep-Copy modules **/
WasmModule *copy_instrument_module(WasmModule *mod) {
    return new WasmModule(*mod);
}

/** Free methods **/
void destroy_instrument_module(WasmModule *mod) { delete mod; }

void destroy_file_buf(byte *file_buf) { free(file_buf); }
