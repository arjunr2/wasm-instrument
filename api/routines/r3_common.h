...
typedef enum { FutexWait = 0, FutexWake = 1, FutexUnknown = -1 } FutexOp;
} // namespace RecordInterface
...
static const char *lockless_import_names[4] = {
    // Futex doesn't need lock since it doesn't need to be ordered w.r.t. memory
    // accesses
    "SYS_futex", "SYS_exit", "__proc_exit", "SYS_exit_group"};
/** */
...
static WasmRet to_wasmret(wasm_type_t type) {
    ...
    case WASM_TYPE_F64:
        return RET_F64;
    default:
        ERR("Type %d does not map to any retval type\n", type);
        return RET_PH;
    }
}
...
static ImportFuncData record_imports[NUM_RECORD_IMPORTS] = {
    {// Tracing function for memory operations
     .iminfo = {.mod_name = "instrument", .member_name = "memop_tracedump"},
     .sig = {.params =
                 {// Differ?
                  WASM_TYPE_I32,
                  // Access Index
    ...
    {// Tracing function for import function calls
     .iminfo = {.mod_name = "instrument", .member_name = "call_tracedump"},
     .sig = {.params =
                 {
                     // Access Index
                     WASM_TYPE_I32,
...
static ImportFuncData replay_imports[NUM_REPLAY_IMPORTS] = {
    {// writev call replay
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_writev"},
     .sig = {.params =
                 {
                     // File descriptor
                     WASM_TYPE_I32,
    ...
    {// proc_exit call replay
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_proc_exit"},
     .sig = {.params =
                 {
                     // Error Code
                     WASM_TYPE_I32,
    ...
    {// thread exit call replay
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_thread_exit"},
     .sig = {.params =
                 {
                     // Error Code
                     WASM_TYPE_I32,
    ...
    {// thread_spawn call replay
     .iminfo = {.mod_name = "wali", .member_name = "__wasm_thread_spawn"},
     .sig = {.params =
    ...
    {// futex operation logging to view synchronization points
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_futex_log"},
     .sig = {.params =
                 {
                     // Futex Address
                     WASM_TYPE_I32,
     ...
     .debug = true},
    {// thread ID tracking
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_gettid"},
     .sig = {.params = {},
             .results =
                 {// Thread ID
                  WASM_TYPE_I32}},
    ...
    {// debug: logging calls performed by replay
     .iminfo = {.mod_name = "r3-replay", .member_name = "SC_log_call"},
     .sig = {.params =
                 {// Access Idx
                  WASM_TYPE_I32,
...
static FuncDecl *add_r3_import_function(WasmModule &module,
                                        ImportFuncData &imdata) {
...
static bool set_func_export_map(WasmModule &module, std::string name,
                                std::map<FuncDecl *, std::string> &export_map) {
...
/* Record currently inserts the import functions (call_tracedump &
  memop_tracedump), making the function indices we get at replay time offset by
  the inserted amount. This method performs a transformation from the index for
  a specific function at replay time to that at record time */
...
static void create_mutex_funcs(WasmModule &module, uint32_t mutex_addr,
                               FuncDecl *funcs[2]) {
    ...
    FuncDecl fn1 = {
        ...
        .num_pure_locals = 0,
        .instructions = {
            INST(BlockInst(void_sig_idx)), INST(LoopInst(void_sig_idx)),
            // Try lock
            INST(I32ConstInst(mutex_addr)), INST(I32ConstInst(0)),
            INST(I32ConstInst(1)), INST(I32AtomicRmwCmpxchgInst(2, 0, memory)),
            //
            INST(I32EqzInst()), INST(BrIfInst(1)),
            // Wait for notify
            INST(I32ConstInst(mutex_addr)), INST(I32ConstInst(1)),
            INST(I64ConstInst(-1)), INST(MemoryAtomicWait32Inst(2, 0, memory)),
            //
            INST(DropInst()), INST(BrInst(0)),

            INST(EndInst()), INST(EndInst()), INST(EndInst())}};
...
static SigDecl get_sigdecl_from_cdef(CSigDecl csig) {
    SigDecl sig;
    for (int i = 0;
         csig.params[i] && (i < sizeof(csig.params) / sizeof(csig.params[0]));
         i++) {
        sig.params.push_back(csig.params[i]);
    }
    for (int i = 0; csig.results[i] &&
                    (i < sizeof(csig.results) / sizeof(csig.results[0]));
         i++) {
