...
struct CallInstInfo {
  ...
  public:
    ...
    void move_into(LocalVal &ret_lc, std::vector<LocalVal> &args,
                   uint16_t opcode, FuncDecl *target,
                   RecordInterface::CallID call_id) {
        this->ret_lc = ret_lc;
...
static std::pair<uint32_t, SigDecl *> get_sig_idx_cache(WasmModule &module,
                                                        SigDecl sig) {
    static std::unordered_map<SigDecl, std::pair<uint32_t, SigDecl *>,
                              SigDeclHash>
        sig_idx_cache;
...
static InstBuilder &builder_push_ifeq_lock(InstBuilder &builder,
                                           ThreadRecordInfo &thread_info,
                                           LocalVal &local_val, uint32_t val) {
    builder.push({INST(BlockInst(-64)), INST(LocalGetInst(local_val.idx)),
                  INST(I32ConstInst(0x7f)), INST(I32AndInst()),
                  INST(I32ConstInst(val)), INST(I32NeInst()), INST(BrIfInst(0)),
                  /* Lock if val == local_val.idx */
                  INST(CallInst(thread_info.lock_fn)), INST(EndInst())});
    return builder;
}
...
static std::pair<bool, LocalVal>
allocate_retval_local(LocalAllocator &lc_allocator, SigDecl *sig) {
    LocalVal ret_lc;
...
static std::vector<LocalVal> save_stack_args(InstBuilder &builder,
    ...
    int i = num_params - 1;
    for (auto xt = values.rbegin(); xt != values.rend(); xt++, i--) {
...
InstBuilder setup_memop_duplicate(
    InstBasePtr &instruction, LocalAllocator &lc_allocator,
    ...
    // Block signature
    SigDecl memop_block_sigdecl = {.params = memop_sig->params,
                                   .results = memop_sig->params};
    auto memop_block_sigpair =
        get_sig_idx_cache(modctx.module, memop_block_sigdecl);
    ...
    for (auto xt = local_vals_allocated.begin();
         xt != local_vals_allocated.end(); xt++) {
        builder.push_inst(LocalGetInst(xt->idx));
    }

    // Default values of record for memory operation (can be updated using
    // `default_updater`)
    ...
    default_updater(new_meminst, modctx, local_vals_allocated, accwidth_lc,
                    mem_offset);
    ...
    recinfo = {.sig = memop_sig,
               .ret_lc = ret_lc,
               .prop = memop_prop,
               .offset = mem_offset,
               .baseaddr_lc = baseaddr_lc,
               .accwidth_lc = accwidth_lc,
               .opcode = opcode,
               .is_sync = is_sync_op};
...
InstBuilder setup_memarg_laneidx_instgen(InstructionContext &instctx,
    ...
    auto updater = [](ImmMemargLaneidxInst *new_inst, ModuleContext &modctx,
                      auto &local_vec, LocalVal &accwidth_lc,
                      auto &_c) { new_inst->setMemory(modctx.record_mem); };
...
InstBuilder setup_memory_instgen(InstructionContext &instctx,
    ...
    return setup_memop_duplicate<ImmMemoryInst>(
        instctx.instruction, lc_allocator, modctx, recinfo, updater, false);
}
...
InstBuilder setup_data_memory_instgen(InstructionContext &instctx,
    ...
    return setup_memop_duplicate<ImmDataMemoryInst>(
        instctx.instruction, lc_allocator, modctx, recinfo, updater, false);
}
...
InstBuilder setup_memorycp_instgen(InstructionContext &instctx,
    ...
    return setup_memop_duplicate<ImmMemorycpInst>(
        instctx.instruction, lc_allocator, modctx, recinfo, updater, false);
}
...
InstBuilder setup_memarg_instgen(InstructionContext &instctx,
    ...
    auto updater = [](ImmMemargInst *new_inst, ModuleContext &modctx, auto _a,
                      auto &_b, uint32_t &mem_offset) {
        new_inst->setMemory(modctx.record_mem);
...
InstBuilder setup_call_instgen(InstructionContext &instctx,
    ...
    auto allocate_arg_locals_instgen = [&lc_allocator, &local_vals_allocated,
                                        &builder, &target_sig] {
        local_vals_allocated =
            save_stack_args(builder, lc_allocator, target_sig->params);
    };
    ...
    if (spec_elem.first) {
        ...
        if (call_name == "SYS_futex") {
            ...
            // Since thread exit ends execution, we need the trace output before
            // the call
            instrument_type.pre_insert_trace = true;
            allocate_arg_locals_instgen();
        } else if ((call_name == "SYS_exit_group") ||
                   (call_name == "__proc_exit")) {
            call_id = RecordInterface::SC_PROC_EXIT;
            // Since proc exit ends execution, we need the trace output before
            // the call
            instrument_type.pre_insert_trace = true;
            allocate_arg_locals_instgen();
        }
    ...
    // Populate the relevant recording info to send to tracing function
    recinfo.move_into(ret_lc, local_vals_allocated, call_opcode, target,
                      call_id);
...
InstBuilder setup_call_indirect_instgen(InstructionContext &instctx,
    ...
    // Access index
    builder.push({INST(LocalSetInst(dyn_func_idx_lc.idx)),
...
InstBuilder callop_trace_instgen(InstructionContext &instctx,
    ...
    // Allocate-on-demand locals (only in specific casseswhen return value is
    // present)
    ...
    SigDecl i64_pr_sigdecl = {.params = {WASM_TYPE_I64},
    ...
    case RecordInterface::SC_MMAP: {
        ...
        builder.push({// Check if memory grew
                      INST(BlockInst(i64_pr_sigpair.first)),
                      INST(LocalTeeInst(i64_tmp_lc.idx)),
                      INST(LocalGetInst(i64_tmp_lc.idx)), INST(I64ConstInst(0)),
                      INST(I64LtSInst()), INST(BrIfInst(0)),
                      // Execute grow in shadow memory
                      INST(MemorySizeInst(modctx.def_mem)),
                      INST(MemorySizeInst(modctx.record_mem)),
                      INST(I32SubInst()), INST(LocalTeeInst(i32_lc_cond_1.idx)),
                      INST(MemoryGrowInst(modctx.record_mem)), INST(DropInst()),
                      // End block
                      INST(EndInst())});
    }
    default: {
    ...
    if (has_ret) {
        builder.push_inst(LocalGetInst(ret_lc.idx)).i64_convert(ret_lc.type);
    } else {
    ...
    auto push_arglocals_i64_extend = [&lc_allocator, &builder, &recinfo,
                                      &num_args]() {
    ...
    case RecordInterface::SC_FUTEX: {
        ...
        builder_push_ifeq_lock(builder, thread_info, recinfo.args[1],
                               RecordInterface::FutexWait);
        break;
    }
    ...
    default: {
        ERR("R3-Record-Error: Unsupported call ID %d\n", call_id);
    }
    }
...
InstBuilder memop_trace_instgen(InstructionContext &instctx,
                                LocalAllocator &lc_allocator,
                                MemopInstInfo &recinfo, FuncDecl *tracedump_fn,
                                ModuleContext &modctx) {
    ...
    builder.push({// Acc Idx
                  ...
                  INST(I32AddInst()), INST(LocalTeeInst(full_addr_lc.idx))});
    ...
    if (recinfo.has_retval()) {
        builder
            ...
            .push({INST(LocalSetInst(i64_shadow_retval_lc.idx)),
                   INST(EndInst())});

        builder.push({// Main memory load value
                      INST(LocalGetInst(i64_main_retval_lc.idx)),
                      // Shadow memory load value
                      INST(LocalGetInst(i64_shadow_retval_lc.idx))});
    } else {
        builder.push({INST(I64ConstInst(0)), INST(I64ConstInst(0))});
    }

    builder.push({// Implicit Sync operation?
...
#define SETUP_INVOKE(ty)                                                       \
    ({                                                                         \
        auto xres =                                                            \
            setup_##ty##_instgen(instctx, lc_allocator, modctx, record.Memop); \
...
void r3_record_instrument(WasmModule &module) {
    ...
    ThreadRecordInfo thread_record_info = {.lock_fn = mutex_funcs[0],
    ...
    // During record: we marshall the access_idx and wrap the original target
    // with a plain `call`.
    std::set<FuncDecl *> funcref_wrappers =
        module.funcref_wrap({WASM_TYPE_I32});
    ...
    func_ignore_map[thread_record_info.lock_fn] =
        "__#internal#_lock_instrument";
    func_ignore_map[thread_record_info.unlock_fn] =
    ...
    for (int i = 0; i < sizeof(specialized_import_names) /
                            sizeof(specialized_import_names[0]);
         i++) {
        if (FuncDecl *f =
                module.find_import_func("wali", specialized_import_names[i]))
    ...
    for (int i = 0;
         i < sizeof(lockless_import_names) / sizeof(lockless_import_names[0]);
         i++) {
        if (FuncDecl *f =
                module.find_import_func("wali", lockless_import_names[i]))
    ...
    ModuleContext modctx = {.module = module,
                            .def_mem = def_mem,
    ...
    for (auto &func : module.Funcs()) {
        ...
        std::map<wasm_type_t, uint32_t> locals_count = {{WASM_TYPE_I32, 7},
                                                        {WASM_TYPE_I64, 6},
                                                        {WASM_TYPE_F32, 3},
                                                        {WASM_TYPE_F64, 3}};
        ...
        for (auto institr = insts.begin(); institr != insts.end(); ++institr) {
            ...
            case IMM_MEMARG:
                setup_builder = SETUP_INVOKE(memarg);
                break;
            case IMM_MEMARG_LANEIDX:
                setup_builder = SETUP_INVOKE(memarg_laneidx);
                break;
            case IMM_MEMORY:
                setup_builder = SETUP_INVOKE(memory);
                break;
            case IMM_DATA_MEMORY:
                setup_builder = SETUP_INVOKE(data_memory);
                break;
            case IMM_MEMORYCP:
                setup_builder = SETUP_INVOKE(memorycp);
                break;
            // Calls: Lock those to select import functions
            case IMM_FUNC: {
                setup_builder = setup_call_instgen(
                    instctx, lc_allocator, call_maps, instrument_type,
                    thread_record_info, modctx, record.Call);
            ...
            case IMM_SIG_TABLE: {
                setup_builder = setup_call_indirect_instgen(
                    instctx, lc_allocator, instrument_type, modctx);
            }
            default: {
            ...
            if (!setup_builder.empty() || instrument_type.force_trace) {
                ...
                case IMM_MEMARG: {
                    trace_builder =
                        memop_trace_instgen(instctx, lc_allocator, record.Memop,
                                            memop_tracedump_fn, modctx);
                    break;
                }
                case IMM_FUNC: {
                    trace_builder = callop_trace_instgen(
                        instctx, lc_allocator, record.Call, call_tracedump_fn,
                ...
                if (!instrument_type.lockless) {
                    post_builder.push_inst(
                        CallInst(thread_record_info.unlock_fn));
                }
