...
std::vector<WasmModule>
memaccess_stochastic_instrument(WasmModule &module, int percent,
...
InstList setup_logappend_args(std::list<InstBasePtr>::iterator &itr,
                              uint32_t local_idxs[7], uint32_t sig_idxs[7],
                              FuncDecl *logaccess_import, uint32_t access_idx,
                              uint32_t mem_base) {
    ...
    std::shared_ptr<ImmMemargInst> mem_inst =
        static_pointer_cast<ImmMemargInst>(instruction);
...
static void insert_logstart_void(WasmModule &module, FuncDecl *main_fn) {
    ImportInfo iminfo = {.mod_name = "instrument", .member_name = "logstart"};
...
static void insert_logend(WasmModule &module, FuncDecl *main_fn) {
    ImportInfo iminfo = {.mod_name = "instrument", .member_name = "logend"};
    ...
    for (auto institr = main_insts.begin(); institr != main_insts.end();
         ++institr) {
        InstBasePtr &instruction = *institr;
        if (instruction->is(WASM_OP_RETURN)) {
            main_insts.insert(institr,
                              InstBasePtr(new CallInst(logend_import)));
        }
    }
...
static std::unordered_map<InstBasePtr, uint32_t>
memaccess_dry_run(WasmModule &module) {
    ...
    TIME_SECTION(
        2, "Time for dry run", for (auto &func : module.Funcs()) {
            InstList &insts = func.instructions;
            for (auto institr = insts.begin(); institr != insts.end();
                 ++institr) {
                InstBasePtr &instruction = *institr;
                if (instruction->getImmType() == IMM_MEMARG) {
                    /* Add instruction will be empty if we choose to ignore some
                     * access instructions eg: atomic.notify, atomic.wait */
                    InstList addinst =
                        setup_logappend_args(institr, local_indices,
                                             sig_indices, NULL, access_idx, 0);
                    if (!addinst.empty()) {
                        access_idx++;
...
static void memfiltered_instrument_internal(WasmModule &module,
    ...
    ImportInfo iminfo = {.mod_name = "instrument", .member_name = "logaccess"};
    SigDecl logaccess_sig = {
        .params = {WASM_TYPE_I32, WASM_TYPE_I32, WASM_TYPE_I32}, .results = {}};
    ImportDecl *logaccess_import_decl =
        module.add_import(iminfo, logaccess_sig);
    ...
    for (auto &func : module.Funcs()) {
        uint32_t local_indices[7] = {
            func.add_local(WASM_TYPE_F64), func.add_local(WASM_TYPE_F32),
            func.add_local(WASM_TYPE_I64), func.add_local(WASM_TYPE_I64),
            func.add_local(WASM_TYPE_I32), func.add_local(WASM_TYPE_I32),
        ...
        for (auto institr = insts.begin(); institr != insts.end(); ++institr) {
            ...
            if (instruction->getImmType() == IMM_MEMARG) {
                /* Add instruction will be empty if we choose to ignore some
                 * access instructions eg: atomic.notify, atomic.wait */
                if (!no_filter && (filter_itr == inst_idx_filter.end())) {
                    break;
                }
                InstList addinst = setup_logappend_args(
                    institr, local_indices, sig_indices, logaccess_import,
                    access_idx, memdata_end);
                if (!addinst.empty()) {
                    // Add if no filtering; or if index matches
                    bool filter_cond =
                        (!no_filter && (*filter_itr == access_idx));
                    if (no_filter || filter_cond) {
                        TRACE("Inserting at Access Idx: %d | Op: %s\n",
                              access_idx,
                              opcode_table[instruction->getOpcode()].mnemonic);
    ...
    if (insert_global) {
        ...
        uint32_t max_inst_id =
            (no_filter
                 ? access_idx
                 : (inst_idx_filter.size() ? *(--inst_idx_filter.end()) : 0));
        GlobalDecl global = {.type = WASM_TYPE_I32,
                             .is_mutable = false,
                             .init_expr_bytes =
                                 INIT_EXPR(I32_CONST, max_inst_id)};
...
memaccess_stochastic_instrument(WasmModule &module, int percent,
    ...
    auto loop = [&module_set, &module, &inst_idx_filter, &max_accesses,
                 &partition_size, &encode_callback](const int a, const int b) {
        for (int i = a; i < b; i++) {
            ...
            std::sample(inst_idx_filter.begin(), inst_idx_filter.end(),
                        ...
                        partition_size, std::mt19937{std::random_device{}()});
...
static std::vector<std::set<uint32_t>> balanced_partition_internal(
    WasmModule &module, int cluster_size,
    std::unordered_map<InstBasePtr, uint32_t> &access_idx_map,
        ...
        for (auto &scope : scope_list) {
            ...
            for (auto institr = scope.start; institr != scope.end; ++institr) {
                ...
                if (instptr->getImmType() == IMM_MEMARG) {
                    uint32_t idx = (access_idx_map.contains(instptr)
                                        ? access_idx_map[instptr]
                                        : 0);
            ...
            for (int i = 0; i < num_hp_size; i++) {
                partitions[sort_idxs[i]].insert(scope_idxs.begin() + dl_idx,
                                                scope_idxs.begin() + dl_idx +
                                                    hp_size);
                partition_weight[sort_idxs[i]] +=
                    (hp_size * (scope.loop_depth + 1));
                dl_idx += hp_size;
            }
