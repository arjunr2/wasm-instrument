...
typedef struct ReplaySigMetaInfo {
  ...
  public:
    ...
    inline void set_block_sigidx(uint32_t ret_sig_indices[4]) {
        ...
        block_sigidx =
            has_retval() ? ret_sig_indices[to_wasmret(get_ret_type()) - RET_I32]
                         : ret_sig_indices[4];
    }
...
static InstBasePtr create_call_to_replay_import(
    ReplayImportFuncDecl &replay_import,
    std::set<ReplayImportFuncDecl> &unused_callid_handlers,
    bool gen_debug_calls) {
    // Create call for debug calls only if flags is set
    bool create_call =
        !(replay_import.debug) || (replay_import.debug && gen_debug_calls);
    if (create_call) {
...
// Operations to push a replay function's return value; defaults to
// 0xDEADBEEFULL
static inline InstBuilder &
builder_push_return_value(InstBuilder &builder, ReplaySigMetaInfo &sigmeta,
...
static inline InstBuilder &builder_push_invalid_case(InstBuilder &builder) {
    builder.push({INST(UnreachableInst()), INST(EndInst())});
    return builder;
}
...
static InstBuilder builder_push_log_call(InstBuilder &builder,
    ...
    builder.push({INST(I32ConstInst(log_info.access_idx)),
    ...
    for (int i = 0;
         i < (sizeof(log_info.call_args) / sizeof(log_info.call_args[0]));
         i++) {
        builder.push_inst(I64ConstInst(log_info.call_args[i]));
    }
    builder.push({INST(I64ConstInst(log_info.sync_id)), log_call_handler_func});
    return builder;
}
...
static InstBuilder builder_push_sync_id_inc(InstBuilder &builder,
    ...
    builder.push({INST(I32ConstInst(sync_id_addr)), INST(I64ConstInst(1)),
                  INST(I64AtomicRmwAddInst(3, 0, def_mem)), INST(DropInst())});
    return builder;
}

static InstBuilder construct_single_replay_prop(
    int prop_arm_idx, ReplayOpProp &prop, ReplaySigMetaInfo &sigmeta,
    bool implicit_sync, MemoryDecl *def_mem,
    std::map<uint32_t, ReplayImportFuncDecl> &callid_handlers,
    std::set<ReplayImportFuncDecl> &unused_callid_handlers, int64_t flags,
    ...
    for (int i = 0; i < (sizeof(prop.call_args) / sizeof(prop.call_args[0]));
         i++) {
        debug_log_call_info.call_args[i] = prop.call_args[i];
    }
    debug_log_call_info.sync_id = prop.sync_id;

    // Sub by 1 right now since sync_id starts with 1; this can be changed in
    // future
    uint64_t cur_prop_sync_id = prop.sync_id - 1;
    // Wait for sync_id to match
    builder.push({INST(I64ConstInst(cur_prop_sync_id)),
                  INST(CallInst(thread_replay_info.wait_sync_id_func))});

    // Generate function for debug logging
    auto log_call_import =
        callid_handlers.find(ReplayInterface::SC_LOG_CALL)->second;
    InstBasePtr log_call_handler_func = create_call_to_replay_import(
        log_call_import, unused_callid_handlers, gen_debug_calls);
    if (log_call_handler_func) {
        builder_push_log_call(builder, log_call_handler_func,
                              debug_log_call_info);
    }

    // Builder for side-effects
    InstBuilder sf_builder = {};
    // Some calls like `exit` end execution; we need to increment sync_id before
    // calling it
    bool pre_insert_sync_inc = false;
    // Implicit sync operations don't actually enforce any props since the
    // actual
    ...
    if (!implicit_sync) {
        ...
        // For imports that require host calls for side-effects, get the
        // appropriate function
        if (callid_func_it != callid_handlers.end()) {
            call_handler_func = create_call_to_replay_import(
                callid_func_it->second, unused_callid_handlers,
                gen_debug_calls);
        ...
        if (gen_side_effects) {
            switch (prop.call_id) {
            case RecordInterface::SC_GENERIC:
                break;
            case RecordInterface::SC_MMAP: {
                uint32_t grow_value = prop.call_args[0];
                if (grow_value > 0) {
                    sf_builder.push({INST(I32ConstInst(grow_value)),
            ...
            case RecordInterface::SC_WRITEV: {
                ...
                sf_builder.push(
                    {INST(I32ConstInst(fd)), INST(I32ConstInst(iov)),
                     INST(I32ConstInst(iovcnt)), call_handler_func});
                break;
            }
            ...
            case RecordInterface::SC_THREAD_EXIT: {
                ...
                sf_builder.push(
                    {INST(I32ConstInst(status)), call_handler_func});
            ...
            case RecordInterface::SC_THREAD_SPAWN: {
                ...
                sf_builder.push({INST(I32ConstInst(setup_fn_ptr)),
            ...
            case RecordInterface::SC_FUTEX: {
                ...
                RecordInterface::FutexOp futex_op =
                    static_cast<RecordInterface::FutexOp>(prop.call_args[1]);
                int32_t val = prop.call_args[2];
                uint64_t sync_id = prop.sync_id;
                switch (futex_op) {
                case RecordInterface::FutexWait: {
                    sf_builder.push(
                        {INST(I32ConstInst(addr)), INST(I32ConstInst(futex_op)),
                         INST(I32ConstInst(val)), call_handler_func});
                    break;
                }
            ...
            default: {
                ERR("R3-Replay-Error: Side-effects unsupported for call_id "
                    "%u\n",
                    prop.call_id);
            }
            }
        ...
        for (int i = 0; i < prop.num_stores; i++) {
            ...
            sf_builder.push({INST(I32ConstInst(store.addr)),
                             INST(I64ConstInst(store.value))});
            switch (store.size) {
            case 1:
                sf_builder.push_inst(I64Store8Inst(0, 0, def_mem));
                break;
            case 2:
                sf_builder.push_inst(I64Store16Inst(0, 0, def_mem));
                break;
            case 4:
                sf_builder.push_inst(I64Store32Inst(0, 0, def_mem));
                break;
            case 8:
                sf_builder.push_inst(I64StoreInst(0, 0, def_mem));
                break;
            default: {
                ERR("R3-Replay-Error: Unsupported size %u", store.size);
            }
            }
        }
    }
    ...
    if (!pre_insert_sync_inc) {
        ...
        builder_push_sync_id_inc(builder, thread_replay_info.sync_id_addr,
                                 def_mem);
    } else {
        builder_push_sync_id_inc(builder, thread_replay_info.sync_id_addr,
                                 def_mem);
...
static InstBuilder construct_thread_replay_props(
    WasmModule &module, InstMetaInfo &imeta_info, int cur_tid, int begin_idx,
    int end_idx, ReplayOpProp *props, ReplaySigMetaInfo &sigmeta,
    bool implicit_sync, MemoryDecl *def_mem,
    std::map<uint32_t, ReplayImportFuncDecl> &callid_handlers,
    std::set<ReplayImportFuncDecl> &unused_callid_handlers, int64_t flags,
    ThreadReplayInfo &thread_replay_info,
    ...
    static GlobalDecl prop_br_tracker_decl = {.type = WASM_TYPE_I32,
                                              .is_mutable = true,
                                              .init_expr_bytes =
    ...
    // Switch table on specific prop with default case, which should never be
    // used
    builder.push_inst(BlockInst(sigmeta.get_block_sigidx()));
    builder_push_return_value(builder, sigmeta);
    builder.push({INST(GlobalGetInst(prop_br_tracker)),
                  INST(BrTableInst(seq_labels, num_props)), INST(EndInst())});

    // Construct each prop arm's replay operations
    for (int i = 0; i < num_props; i++) {
        InstBuilder prop_builder = construct_single_replay_prop(
            num_props - i - 1, props[begin_idx + i], sigmeta, implicit_sync,
            def_mem, callid_handlers, unused_callid_handlers, flags,
            thread_replay_info, debug_log_call_info);
    ...
    builder.push({INST(GlobalGetInst(prop_br_tracker)), INST(I32ConstInst(1)),
                  INST(I32AddInst()), INST(GlobalSetInst(prop_br_tracker)),
                  INST(EndInst())});
...
static void replace_import_call(WasmModule &module, InstBuilder &builder,
    ...
    if (implicit_sync) {
        sprintf(replay_func_debug_name, "__replay_instrument_#-#-%u",
                access_idx);
    } else {
        ImportDecl *import_decl =
            module.get_import_name_from_func(imeta_info.target_func);
        sprintf(replay_func_debug_name, "__replay_instrument_%s-%s-%u",
                import_decl->mod_name.c_str(), import_decl->member_name.c_str(),
                access_idx);
        remove_importfunc = true;
    }

    FuncDecl replay_ops_func_decl = {.sig = sigmeta.get_inst_sig(),
                                     .pure_locals = wasm_localcsv_t(),
                                     .num_pure_locals = 0,
                                     .instructions = builder.get_buf()};
    FuncDecl *replay_ops_func =
        module.add_func(replay_ops_func_decl, NULL, replay_func_debug_name);
...
/* Inserts the return value and store replay operations (for all threads)
   ...
   */
static void
insert_replay_op(WasmModule &module, InstMetaInfo &imeta_info, ReplayOp &op,
                 uint32_t ret_sig_indices[4],
                 std::set<FuncDecl *> &remove_importfuncs,
                 std::map<uint32_t, ReplayImportFuncDecl> &callid_handlers,
                 std::set<ReplayImportFuncDecl> &unused_callid_handlers,
    ...
    if (implicit_sync) {
        ...
        SigDecl s =
            get_sigdecl_from_cdef(memop_inst_table[imeta_info.opcode].sig);
        sigmeta.set_inst_sig(module.add_sig(s, false));
    } else {
        // Verify that import function and the indices are consistent with
        // record before proceeding
        if (!empty_replay_op) {
            uint32_t replay_func_idx = module.getFuncIdx(target_func);
            uint32_t record_func_idx =
                transform_to_record_func_idx(replay_func_idx);
    ...
    {
        SigDecl block_sig = {.params = {},
                             .results = {sigmeta.get_inst_sig()->results}};
    ...
    if (empty_replay_op) {
        ...
        if (thread_replay_info.is_multithreaded) {
            ...
            // Thread Replay Info: Includes an error case if tid is outside of
            // [0, max_tid]
            for (int i = 0; i < num_tid_arms; i++) {
                builder.push_inst(BlockInst(sigmeta.get_block_sigidx()));
            }

            // Switch table on thread id with default case, which should never
            // be used
            auto gettid_import =
                callid_handlers.find(ReplayInterface::SC_GETTID)->second;
            builder.push_inst(BlockInst(sigmeta.get_block_sigidx()));
            // This is for type-safety of block; should never be returned from
            // table
            builder_push_return_value(builder, sigmeta);
            builder.push({create_call_to_replay_import(
                              gettid_import, unused_callid_handlers, true),
            ...
            for (int cur_tid = first_tid, i = 0; i < op.num_props; cur_tid++) {
                ...
                int begin_idx = i;
                while ((i < op.num_props) && (op.props[i].tid == cur_tid)) {
                    i++;
                }
                int end_idx = i;
                ...
                InstBuilder tid_props = construct_thread_replay_props(
                    ...
                    sigmeta, implicit_sync, module.getMemory(0),
                    callid_handlers, unused_callid_handlers, flags,
                    thread_replay_info, debug_log_call_info);
        ...
        // Single-threaded replays can avoid gettid and tid switch table
            ...
            InstBuilder single_thread_props = construct_thread_replay_props(
                ...
                sigmeta, implicit_sync, module.getMemory(0), callid_handlers,
    ...
    replace_import_call(module, builder, imeta_info, op.access_idx,
                        remove_importfuncs, sigmeta, implicit_sync);
}

/* Create function to implement synchronization of program to deterministic
 * trace order */
/* Function waits until sync_id_addr contains param0 in it */
static FuncDecl *create_wait_sync_id_func(WasmModule &module,
                                          uint32_t sync_id_addr) {
    ...
    FuncDecl fn = {
        ...
        .num_pure_locals = 0,
        .instructions = {
            INST(LoopInst(-0x40)), INST(I32ConstInst(sync_id_addr)),
            INST(I64AtomicLoadInst(3, 0, memory)),
            INST(LocalGetInst(0)) /* sync_id */, INST(I64NeInst()),
            INST(BrIfInst(0)), /* Retry until M[sync_id_addr] != sync_id */
            INST(EndInst()), INST(EndInst())}};
...
void r3_replay_instrument(WasmModule &module, void *replay_ops,
    ...
    // During replay: access_idx is unneccessary and import calls will be
    // replaced as needed
    ...
    for (int i = 0; i < num_ops; i++) {
        thread_replay_info.num_threads =
            std::max(thread_replay_info.num_threads, ops[i].max_tid);
    }
    ...
    /* Do not instrument the instrument-hooks or start (usually just
     * initialization) */
    func_ignore_map[module.get_start_fn()] = "__#internal#_start_function";
    func_ignore_map[thread_replay_info.lock_func] =
        "__#internal#_lock_instrument";
    func_ignore_map[thread_replay_info.unlock_func] =
        "__#internal#_unlock_instrument";
    ...
    for (auto &func : module.Funcs()) {
        ...
        for (auto institr = insts.begin(); institr != insts.end(); ++institr) {
            ...
            InstMetaInfo imeta_info = {.opcode = instruction->getOpcode(),
                                       .institr = institr,
                                       .insts = insts,
            ...
            case IMM_FUNC: {
                ...
                imeta_info.is_import_target =
                    module.isImport(imeta_info.target_func);
                break;
            }
            case IMM_MEMARG:
            ...
            ReplayOp curr_op = {.access_idx = cur_access_tracker,
                                .func_idx = 0,
                                .props = nullptr,
                                .num_props = 0};

            bool is_tracked_access_idx =
                (op_idx < num_ops) &&
            ...
            if (run_replay_replacement) {
                insert_replay_op(module, imeta_info, curr_op, sig_indices,
                                 remove_importfuncs, callid_handlers,
                                 unused_callid_handlers, flags,
                                 thread_replay_info, debug_log_call_info);
                TRACE("[%s] Replaced Call Access %d\n",
                      (is_tracked_access_idx ? "TRACKED" : "UNTRACKED"),
                      cur_access_tracker);
            }
        }
    }
