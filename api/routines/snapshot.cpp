...
void snapshot_exit_insert(WasmModule &module, MemoryDecl *memory, uint32_t instmem_start) {
...
void open_callback(WasmModule &module, FuncDecl *src_func, FuncDecl *call_func) {
    TypeList x = call_func->sig->params;
    printf("Callback called\n");
}

void open_insert(WasmModule &module) {
    ImportInfo open_info = { .mod_name = "wasi_snapshot_preview1", .member_name = "path_open" };
    FuncDecl *open_fn = module.find_import_func(open_info.mod_name, open_info.member_name);
    if (open_fn == NULL) {
        return;
    }

    std::map<FuncDecl*, DirectCallSiteCallback> openmap = { {open_fn, open_callback} };
