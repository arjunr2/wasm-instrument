...
std::string allspark_layout_inspect(WasmModule &module) {
    ...
    for (auto &func : module.Funcs()) {
        ...
        // To determine how to render instructions when you first enter a
        // function
        ...
        auto move_br_blocks = [&br_blocks, &inst_entries,
                               &target_sites](std::string pc) {
            br_blocks[pc] = std::move(inst_entries);
            uint32_t i = 0;
            bool first = true;
            for (auto &inst : br_blocks[pc]) {
                if ((inst["type"] == TARGET_INST_TYPE) || first) {
                    target_sites[inst["pc"]] = {{"block", pc}, {"idx", i}};
                    first = false;
                }
                i++;
            }
        ...
        for (auto &inst : func.instructions) {
            ...
            case WASM_OP_CALL: {
                ...
                call_direct_sites[pc] =
                    std::to_string(module.getFuncIdx(call_inst->getFunc()));
                break;
            }
            case WASM_OP_RETURN: {
