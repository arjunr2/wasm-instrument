...
static void insert_opcode_instrumentation(WasmModule &module, InstItr &institr, FuncDecl* func) {
  static GlobalDecl global = {
    .type = WASM_TYPE_I64,
    .is_mutable = true,
  ...
  for (auto it = run_counts.begin(); it != run_counts.end(); ++it) {
    ...
    if (!(gref = found_ops[opcode])) {
      gref = found_ops[opcode] = module.add_global(global,
...
static void scope_opcode_instrument (WasmModule &module, ScopeBlock* scope, FuncDecl* func) {
  ...
  for (institr = scope->start; institr != scope->end; ++institr) {
    ...
    if ( (next_outer_subscope != scope->outer_subscopes.end()) && 
      ...
      if ((instptr->isImmType(IMM_LABEL) || instptr->isImmType(IMM_LABELS)) ||
