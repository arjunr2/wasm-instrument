...
// Instrumentation to generate wrappers on funcref elements in tables  (works only on active elems)
// Critical component to enable 'call_indirect' instrumentation.
// Wrapper can be instrumented to dynamically capture indirect calls.
// Extra contextual data can be marshalled with 'marshall_vals' to the wrapper
// Returns set of added wrapper functions
std::set<FuncDecl*> WasmModule::funcref_wrap (std::vector<wasm_type_t> marshall_vals) {
    ...
    for (auto &elem: this->Elems()) {
        ...
        for (auto &funcref: elem.funcs) {
            ...
            FuncDecl new_fn_decl = {
                .sig = this->add_sig(new_fn_sig, false),
            ...
            FuncDecl *new_fn = this->add_func(new_fn_decl, NULL, new_dname.c_str());
...
// Append instructions at all points before a module exits (end of entry functions)
// Can include an list of import functions as "exit points"
// NOTE: Implicitly transforms indirect calls to direct calls 
void WasmModule::exit_instrument(InstBuilder &builder, std::vector<ImportInfo> import_func_exits, FuncDecl* entry_func) {
...
void WasmModule::beforeImmTypeInstruction(opcode_imm_type imm_type, InstructionSiteCallback callback) {
    for (auto &func: this->Funcs()) {
        for (auto institr = func.instructions.begin(); institr != func.instructions.end(); ++institr) {
            InstBasePtr &instptr = *institr;
...
void WasmModule::beforeFuncCall(std::map<FuncDecl*, DirectCallSiteCallback> &builder_map) {
    auto x = [&builder_map](WasmModule &module, FuncDecl *func, InstBasePtr instptr) {
