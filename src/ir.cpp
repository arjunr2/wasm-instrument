...
const char *wasm_section_name(byte code) {
    ...
    case WASM_SECT_DATA:
        return "data";
    case WASM_SECT_DATACOUNT:
        return "datacount";
    case WASM_SECT_CUSTOM:
        return "custom";
    default:
        return "unknown";
    }
...
uint32_t MemoryDecl::add_pages(uint32_t num_pages) {
    ...
    if (memlimit.has_max && (new_size > memlimit.max)) {
        WARN("Adding more pages (%u + %u) than current max memory size (%u); "
             ...
             memlimit.initial, num_pages, memlimit.max);
...
WasmModule &WasmModule::deepcopy(const WasmModule &mod, const char *log_str) {
    ...
    this->start_fn =
        mod.start_fn ? REASSIGN(mod.start_fn, Func, FuncDecl) : NULL;
...
template <typename T>
void WasmModule::DescriptorPatch(
    std::list<T> &list, const WasmModule &mod,
    std::unordered_map<void *, void *> &reassign_cache) {
    for (auto &v : list) {
        switch (v.kind) {
        case KIND_FUNC:
            v.desc.func = REASSIGN(v.desc.func, Func, FuncDecl);
            break;
        case KIND_TABLE:
            v.desc.table = REASSIGN(v.desc.table, Table, TableDecl);
            break;
        case KIND_MEMORY:
            v.desc.mem = REASSIGN(v.desc.mem, Memory, MemoryDecl);
            break;
        case KIND_GLOBAL:
            v.desc.global = REASSIGN(v.desc.global, Global, GlobalDecl);
            break;
        default:
...
#define PCS(cs, clsname, ...)                                                  \
    case cs: {                                                                 \
        ...
        instptr.reset(                                                         \
            new clsname(instptr->getOpcode() __VA_OPT__(, ) __VA_ARGS__));     \
        break;                                                                 \
    }
void WasmModule::FunctionPatch(
    const WasmModule &mod, std::unordered_map<void *, void *> &reassign_cache) {
    DEF_TIME_VAR();

    TIME_SECTION(
        3, "Time for func patching", for (auto &func : this->funcs) {
            ...
            for (auto institr = func.instructions.begin();
                ...
                switch (instptr->getImmType()) {
                    ...
                    PCS(IMM_LABELS, ImmLabelsInst, ispec->getLabels(),
                        ispec->getDefLabel());
                    PCS(IMM_FUNC, ImmFuncInst,
                        REASSIGN(ispec->getFunc(), Func, FuncDecl));
                    PCS(IMM_SIG_TABLE, ImmSigTableInst,
                        REASSIGN(ispec->getSig(), Sig, SigDecl),
                        REASSIGN(ispec->getTable(), Table, TableDecl));
                    PCS(IMM_LOCAL, ImmLocalInst, ispec->getLocal());
                    PCS(IMM_GLOBAL, ImmGlobalInst,
                        REASSIGN(ispec->getGlobal(), Global, GlobalDecl));
                    PCS(IMM_TABLE, ImmTableInst,
                        REASSIGN(ispec->getTable(), Table, TableDecl));
                    PCS(IMM_MEMARG, ImmMemargInst, ispec->getAlign(),
                        ispec->getOffset(),
                        REASSIGN(ispec->getMemory(), Memory, MemoryDecl));
                    PCS(IMM_I32, ImmI32Inst, ispec->getValue());
                    PCS(IMM_F64, ImmF64Inst, ispec->getValue());
                    PCS(IMM_MEMORY, ImmMemoryInst,
                        REASSIGN(ispec->getMemory(), Memory, MemoryDecl));
                    ...
                    PCS(IMM_DATA_MEMORY, ImmDataMemoryInst,
                        REASSIGN(ispec->getData(), Data, DataDecl),
                        REASSIGN(ispec->getMemory(), Memory, MemoryDecl));
                    PCS(IMM_DATA, ImmDataInst,
                        REASSIGN(ispec->getData(), Data, DataDecl));
                    PCS(IMM_MEMORYCP, ImmMemorycpInst,
                        REASSIGN(ispec->getDstMemory(), Memory, MemoryDecl),
                        REASSIGN(ispec->getSrcMemory(), Memory, MemoryDecl));
                    PCS(IMM_DATA_TABLE, ImmDataTableInst,
                        REASSIGN(ispec->getData(), Data, DataDecl),
                        REASSIGN(ispec->getTable(), Table, TableDecl));
                    PCS(IMM_TABLECP, ImmTablecpInst,
                        REASSIGN(ispec->getDstTable(), Table, TableDecl),
                        REASSIGN(ispec->getSrcTable(), Table, TableDecl));
                    ...
                    PCS(IMM_MEMARG_LANEIDX, ImmMemargLaneidxInst,
                        ispec->getAlign(), ispec->getOffset(),
                        REASSIGN(ispec->getMemory(), Memory, MemoryDecl),
                        ispec->getLaneidx());
                }
            }
