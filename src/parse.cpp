...
inline static wasm_limits_t read_limits(buffer_t &buf) {
    ...
    uint32_t min = RD_U32();
    wasm_limits_t limits = {.initial = min,
...
inline static TableDecl read_tabletype(buffer_t &buf) {
    ...
    TableDecl table = {.reftype = (wasm_type_t)reftype,
                       .limits = read_limits(buf)};
    return table;
}
...
inline static GlobalDecl read_globaltype(buffer_t &buf) {
    ...
    GlobalDecl glob = {.type = (wasm_type_t)type, .is_mutable = is_mutable};
    return glob;
}
    ...
    for (uint32_t i = 0; i < num_sigs; i++) {
        ...
        if (type != WASM_TYPE_FUNC) {
            throw std::runtime_error(
                "Parse error: Signatures have to be func type!\n");
        }
...
void WasmModule::decode_element_section(buffer_t &buf, uint32_t len) {
    ...
    for (uint32_t i = 0; i < num_elem; i++) {
        ...
        switch (flag) {
        case 0:
            offset = decode_const_off_expr(buf, elem.opcode_offset);
            break;
        default: {
    ...
    if (this->has_datacount) {
        if (num_datas != this->num_datas_datacount) {
            ERR("Num datas: %u | Num datacount: %u\n", num_datas,
                this->num_datas_datacount);
            throw std::runtime_error(
                "Datacount section and data section vector "
                "size don't match");
        }
    }
        ...
        while (buf.ptr != end_sec) {
            ...
            /* Function subsection */
                ...
                for (uint32_t i = 0; i < num_names; i++) {
                    uint32_t idx = RD_U32();
                    DebugNameAssoc d = {.func = this->getFunc(idx),
                                        .name = RD_NAME()};
                    debug.func_assoc.push_back(d);
                }

                if (buf.ptr != end_subsec) {
                    ERR("Custom name section not aligned after parsing -- "
                        "start:%lu, "
                        "ptr:%lu, end:%lu\n",
                        buf.ptr - start_subsec, buf.ptr - start_subsec,
...
#define DECODE_CALL(sec, ...)                                                  \
    this->decode_##sec##_section(cbuf, len __VA_OPT__(, ) __VA_ARGS__);        \
    break;
        switch (section_id) {
        case WASM_SECT_TYPE:
            this->decode_type_section(cbuf, len);
            break;
        case WASM_SECT_IMPORT:
            this->decode_import_section(cbuf, len);
            break;
        case WASM_SECT_FUNCTION:
            this->decode_function_section(cbuf, len);
            break;
        case WASM_SECT_TABLE:
            this->decode_table_section(cbuf, len);
            break;
        case WASM_SECT_MEMORY:
            this->decode_memory_section(cbuf, len);
            break;
        case WASM_SECT_GLOBAL:
            this->decode_global_section(cbuf, len);
            break;
        case WASM_SECT_EXPORT:
            this->decode_export_section(cbuf, len);
            break;
        case WASM_SECT_START:
            this->decode_start_section(cbuf, len);
            break;
        case WASM_SECT_ELEMENT:
            this->decode_element_section(cbuf, len);
            break;
        case WASM_SECT_CODE:
            this->decode_code_section(cbuf, len, gen_cfg);
            break;
        case WASM_SECT_DATA:
            this->decode_data_section(cbuf, len);
            break;
        case WASM_SECT_DATACOUNT:
            this->decode_datacount_section(cbuf, len);
            break;
        case WASM_SECT_CUSTOM:
            this->decode_custom_section(cbuf, len);
            break;
        default:
        ...
        if (cbuf.ptr != cbuf.end) {
            ERR("Section \"%s\" not aligned after parsing -- start:%lu, "
                "ptr:%lu, "
                "end:%lu\n",
                wasm_section_name(section_id), cbuf.start - buf.start,
                cbuf.ptr - buf.start, cbuf.end - buf.start);
            throw std::runtime_error("Section parsing error");
        }
        // Advance section
        buf.ptr = cbuf.ptr;
