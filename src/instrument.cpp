...
#define IMPORT_INJ_KIND_FUNC(objref)                                           \
    {                                                                          \
        if (this->fn_names_debug) {                                            \
            DebugNameAssoc dname = {.func = &this->funcs.front(),              \
...
#define IMPORT_ADD(kd, field, decl)                                            \
    {                                                                          \
        ...
        ImportDecl import = {.mod_name = info.mod_name,                        \
...
ImportDecl *WasmModule::add_import(ImportInfo &info, GlobalInfo &global) {
    GlobalDecl gdecl = {.type = global.type, .is_mutable = global.is_mutable};
...
#define EXPORT_ADD(kd, field)                                                  \
    {                                                                          \
        ...
        ExportDecl exp = {                                                     \
            .name = export_name, .kind = kd, .desc = {.field = &field}};       \
        exports.push_back(exp);                                                \
        return &exports.back();                                                \
    }

// Exports for all declarations
ExportDecl *WasmModule::add_export(std::string export_name,
                                   GlobalDecl &global) {
...
#define IMPORT_FIND(kd, field)                                                 \
    {                                                                          \
        for (auto &import : this->imports.list) {                              \
            if (import.mod_name == mod_name &&                                 \
                import.member_name == member_name && kd == import.kind)        \
                return import.desc.field;                                      \
        }                                                                      \
        return NULL;                                                           \
    }

GlobalDecl *WasmModule::find_import_global(std::string mod_name,
                                           std::string member_name) {
    IMPORT_FIND(KIND_GLOBAL, global);
}
TableDecl *WasmModule::find_import_table(std::string mod_name,
                                         std::string member_name) {
    IMPORT_FIND(KIND_TABLE, table);
}
MemoryDecl *WasmModule::find_import_memory(std::string mod_name,
                                           std::string member_name) {
    IMPORT_FIND(KIND_MEMORY, mem);
}
FuncDecl *WasmModule::find_import_func(std::string mod_name,
                                       std::string member_name) {
...
WasmModule::get_debug_name_from_func(FuncDecl *func) {
    if (auto debug_names_ptr = this->getFnDebugNames()) {
...
bool WasmModule::remove_func_core(uint32_t idx, FuncDecl *func) {
    ...
    if (auto debug_names_ptr = this->getFnDebugNames()) {
        auto &debug_names = *debug_names_ptr;
        for (auto dname_itr = debug_names.begin();
             dname_itr != debug_names.end(); ++dname_itr) {
