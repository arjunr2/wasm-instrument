...
ImmBlocktInst::ImmBlocktInst(WasmModule &module, Opcode_t opcode, buffer_t &buf)
    ...
    if (value_type >= 0) {
        ...
        throw std::runtime_error(
            "Block type with type section index unsupported!");
    }
...
std::string ImmBlocktInst::to_string(WasmModule &module) const {
    return sprintf_string("%s type[%ld]", this->opString().c_str(),
                          this->getType());
}
...
std::string ImmLabelsInst::to_string(WasmModule &module) const {
    ...
    return sprintf_string("%s %s %ld", this->opString().c_str(), labels.c_str(),
                          this->def_idx);
}
...
/* ImmSigTableInst  */
ImmSigTableInst::ImmSigTableInst(WasmModule &module, Opcode_t opcode,
                                 buffer_t &buf)
    : InstBase(opcode) {
...
std::string ImmMemargInst::to_string(WasmModule &module) const {
    return sprintf_string("%s [%d] offset=%ld align=%ld",
                          this->opString().c_str(),
                          module.getMemoryIdx(this->getMemory()),
                          this->getOffset(), this->getAlign());
}
...
/* ImmRefnulltInst  */
ImmRefnulltInst::ImmRefnulltInst(WasmModule &module, Opcode_t opcode,
                                 buffer_t &buf)
    : InstBase(opcode) {
...
/* ImmDataMemoryInst  */
ImmDataMemoryInst::ImmDataMemoryInst(WasmModule &module, Opcode_t opcode,
                                     buffer_t &buf)
    : InstBase(opcode) {
...
/* ImmMemorycpInst  */
ImmMemorycpInst::ImmMemorycpInst(WasmModule &module, Opcode_t opcode,
                                 buffer_t &buf)
    : InstBase(opcode) {
...
/* ImmDataTableInst  */
ImmDataTableInst::ImmDataTableInst(WasmModule &module, Opcode_t opcode,
                                   buffer_t &buf)
    : InstBase(opcode) {
    this->data = module.getData(RD_U32());
    this->table = module.getTable(RD_U32());
...
/* ImmTablecpInst  */
ImmTablecpInst::ImmTablecpInst(WasmModule &module, Opcode_t opcode,
                               buffer_t &buf)
    : InstBase(opcode) {
...
/* ImmLaneidxInst  */
ImmLaneidxInst::ImmLaneidxInst(WasmModule &module, Opcode_t opcode,
                               buffer_t &buf)
    : InstBase(opcode) {
    this->idx = RD_BYTE();
}
...
std::string ImmLaneidxInst::to_string(WasmModule &module) const {
    return sprintf_string("%s %u", this->opString().c_str(),
                          this->getLaneidx());
}

/* ImmLaneidx16Inst  */
ImmLaneidx16Inst::ImmLaneidx16Inst(WasmModule &module, Opcode_t opcode,
                                   buffer_t &buf)
    : InstBase(opcode) {
    for (int i = 0; i < 16; i++) {
...
ImmMemargLaneidxInst::ImmMemargLaneidxInst(WasmModule &module, Opcode_t opcode,
                                           buffer_t &buf)
    : InstBase(opcode) {
    uint32_t align = RD_U32();
    bool has_memidx = ((align >> 6) & 1);
...
void ImmMemargLaneidxInst::encode_imm(WasmModule &module,
                                      bytedeque &bdeq) const {
    uint32_t memidx = module.getMemoryIdx(this->mem);
    if (memidx) {
        WR_U32((this->align | 0x40));
        WR_U32(module.getMemoryIdx(this->mem));
...
std::string ImmMemargLaneidxInst::to_string(WasmModule &module) const {
    return sprintf_string("%s [] [] offset=%d align=%d",
                          module.getMemoryIdx(this->mem), this->laneidx,
