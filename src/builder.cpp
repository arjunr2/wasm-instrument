...
InstBuilder &InstBuilder::i64_convert(wasm_type_t type) {
    switch (type) {
    case WASM_TYPE_I32:
        this->push_inst(I64ExtendI32UInst());
        break;
    case WASM_TYPE_I64:;
        break;
    case WASM_TYPE_F32:
        this->push({INST(I32ReinterpretF32Inst()), INST(I64ExtendI32UInst())});
        break;
    case WASM_TYPE_F64:
        this->push_inst(I64ReinterpretF64Inst());
        break;
    default: {
...
InstBuilder &InstBuilder::ne(wasm_type_t type) {
    auto neq = [&type] {
        switch (type) {
        case WASM_TYPE_I32:
            return INST(I32NeInst());
        case WASM_TYPE_I64:
            return INST(I64NeInst());
        case WASM_TYPE_F32:
            return INST(F32NeInst());
        case WASM_TYPE_F64:
            return INST(F64NeInst());
        default: {
...
InstBuilder &InstBuilder::i64_store(uint32_t size, MemoryDecl *mem) {
    auto store = [&size, &mem] {
        switch (size) {
        case 1:
            return INST(I64Store8Inst(0, 0, mem));
        case 2:
            return INST(I64Store16Inst(0, 0, mem));
        case 4:
            return INST(I64Store32Inst(0, 0, mem));
        case 8:
            return INST(I64StoreInst(0, 0, mem));
        default: {
